...
Bioregeneration research is complete. It runs for 12 days.

For reference, the vanilla cycles are 4 days for pleasure, 6 for medic, 8 for age reversal and 25
for bioregeneration. Detox is the second longest thing a pod can do.
...
**Not luciferium, and not anything else the game calls permanent.** The exclusion is the game's own
`everCurableByItem` flag rather than a name this mod knows, so a modded permanent addiction is
excluded too.
...
- No settings. The duration, and whether luciferium is excluded, are not configurable, and there is
  no settings screen to put them on.
- No severity scaling. Every cycle is 12 days regardless of how addicted the pawn is.
...
Biotech's genetic chemical dependency is deliberately untouched. It is a different type and the
game flags it incurable; removing it would kill the pawn, because the gene re-adds it.
